...
public class EquipmentAllocationController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("activities",
					dao.findActivitiesForMentor(mentorId, request.getParameter("keyword"), status,
							optionalDate(request.getParameter("fromDate")),
							optionalDate(request.getParameter("toDate"))));
			...
			switch (action) {
				...
				case "reviewIssue" -> {
					...
					if (!"VERIFIED".equals(decision) && !"REJECTED".equals(decision))
						throw new IllegalArgumentException("Quyết định xử lý sự cố không hợp lệ.");
					dao.reviewIssue(id(request, "issueId"), mentorId, "VERIFIED".equals(decision),
							request.getParameter("mentorNote"));
				}
				default -> {
	...
	private void showActivity(HttpServletRequest request, HttpServletResponse response, long activityId)
	...
	private void showActivityForm(HttpServletRequest request, HttpServletResponse response, EquipmentActivity activity,
			String formMode) throws SQLException, ServletException, IOException {
		request.setAttribute("activity", activity);
	...
	private List<EquipmentAllocationRequest> classRequestRows(HttpServletRequest request) {
		...
		for (int index = 0; index < assetIds.length; index++) {
			...
			try {
				...
				throw new IllegalArgumentException("Tài sản và số lượng phải hợp lệ.");
			}
	...
	private String message(Exception exception) {
		return exception.getMessage() == null ? "Không thể thực hiện thao tác." : exception.getMessage();
	}
}

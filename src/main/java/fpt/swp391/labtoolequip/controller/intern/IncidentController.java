...
public class IncidentController extends HttpServlet {
		...
		try {
			...
			if (path != null && path.matches("/\\d+")) {
				request.setAttribute("incident",
						incidentDAO.findByIdForIntern(Long.parseLong(path.substring(1)), AuthSession.userId(request))
								.orElseThrow());
			...
			request.setAttribute("incidents", incidentDAO.findForIntern(AuthSession.userId(request),
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			long incidentId = incidentDAO.createForIntern(AuthSession.userId(request), usageId(request),
					request.getParameter("incidentType"), request.getParameter("severity"), occurredAt(request),
					request.getParameter("description"));
			response.sendRedirect(request.getContextPath() + "/intern/incidents/" + incidentId + "?created=1");
		} catch (SQLException exception) {
			throw new ServletException(exception);
	...
	private void showForm(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("csrfToken", Csrf.token(request));
		request.setAttribute("usages", usageDAO.findForIntern(AuthSession.userId(request), "", ""));
		forward(request, response, "form.jsp");
	}

	private long usageId(HttpServletRequest request) {
		try {
			long value = Long.parseLong(request.getParameter("usageId"));
			if (value <= 0)
				throw new NumberFormatException();
			return value;
		} catch (NumberFormatException exception) {
			throw new IllegalArgumentException("Lượt sử dụng không hợp lệ.");
		}
	}
	...
	private void forward(HttpServletRequest request, HttpServletResponse response, String view)
			throws ServletException, IOException {
		request.getRequestDispatcher("/WEB-INF/views/intern/incidents/" + view).forward(request, response);
	}
}

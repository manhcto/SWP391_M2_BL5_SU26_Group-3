    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (path != null && path.matches("/\\d+")) {
                ...
                if (allocationRequest == null) { response.sendError(HttpServletResponse.SC_NOT_FOUND); return; }
                ...
                request.getRequestDispatcher("/WEB-INF/views/labmanager/allocations/detail.jsp").forward(request, response);
                return;
            }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if ("approve".equals(request.getParameter("action"))) {
                ...
                dao.approveRequest(requestId, AuthSession.userId(request), request.getParameterValues("assetItemId"), request.getParameter("reviewNote"));
                redirect += "/" + requestId;
            } else if ("recover".equals(request.getParameter("action"))) {
                dao.recoverAllocation(id(request, "allocationId"), AuthSession.userId(request), request.getParameter("returnCondition"), request.getParameter("returnNote"));
            } else { response.sendError(HttpServletResponse.SC_BAD_REQUEST); return; }
            response.sendRedirect(redirect + "?success=1");
        } catch (IllegalArgumentException | SQLException exception) {
            response.sendRedirect(redirect + "?error=" + URLEncoder.encode(exception.getMessage(), StandardCharsets.UTF_8));
        }
    }

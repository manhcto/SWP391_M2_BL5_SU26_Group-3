                ...
                <article class="panel">
                    ...
                    <div class="table-scroll inspection-form-scroll"><table class="inspection-table inspection-item-table"><thead><tr><th>Chọn</th><th>Thiết bị</th><th>Số lượng dự kiến</th><th>Số lượng thực tế</th><th>Tình trạng dự kiến</th><th>Tình trạng thực tế</th><th>Loại chênh lệch</th><th>Ghi chú chênh lệch</th></tr></thead><tbody>
                        <c:forEach var="asset" items="${assets}">
                            <c:set var="rowKey" value="${asset.rowKey}"/>
                            <c:set var="item" value="${itemByTarget[rowKey]}"/>
                            <c:set var="checked" value="${inspection.scope == 'WHOLE_LAB' || not empty item}"/>
                            <c:set var="serialized" value="${not empty asset.assetItemId}"/>
                            <tr data-tracking-mode="${asset.trackingMode}">
                                <td><input class="asset-check" type="checkbox" name="selectedAssetId" value="${asset.assetId}" ${checked ? 'checked' : ''}><input type="hidden" name="targetKey" value="${rowKey}"><input type="hidden" name="assetId_${rowKey}" value="${asset.assetId}"><input type="hidden" name="assetItemId_${rowKey}" value="${asset.assetItemId}"></td>
                                <td class="asset-cell"><strong><c:out value="${asset.assetCode}"/></strong><small><c:out value="${asset.assetName}"/> · <c:out value="${asset.trackingMode == 'SERIALIZED' ? 'Quản lý riêng lẻ' : 'Theo số lượng'}"/><c:if test="${not empty asset.assetItemCode}"> · Item: <c:out value="${asset.assetItemCode}"/></c:if><c:if test="${not empty asset.serialNumber}"> · Serial: <c:out value="${asset.serialNumber}"/></c:if></small></td>
                                <td><input class="form-control compact-input" type="number" min="${serialized ? 1 : 0}" max="${serialized ? 1 : ''}" name="expectedQuantity_${rowKey}" value="${serialized ? 1 : (empty item ? asset.expectedQuantity : item.expectedQuantity)}" ${serialized ? 'readonly' : ''}></td>
                                <td><input class="form-control compact-input" type="number" min="0" max="${serialized ? 1 : ''}" name="actualQuantity_${rowKey}" value="${serialized ? (empty item ? 1 : item.actualQuantity) : (empty item ? asset.actualQuantity : item.actualQuantity)}"></td>
                                <td><select class="form-control" name="expectedCondition_${rowKey}"><option value="">-</option><option value="GOOD" ${(empty item && asset.expectedCondition == 'GOOD') || item.expectedCondition == 'GOOD' ? 'selected' : ''}>Tốt</option><option value="FAIR" ${(empty item && asset.expectedCondition == 'FAIR') || item.expectedCondition == 'FAIR' ? 'selected' : ''}>Khá</option><option value="DAMAGED" ${(empty item && asset.expectedCondition == 'DAMAGED') || item.expectedCondition == 'DAMAGED' ? 'selected' : ''}>Hư hỏng</option><option value="BROKEN" ${(empty item && asset.expectedCondition == 'BROKEN') || item.expectedCondition == 'BROKEN' ? 'selected' : ''}>Không hoạt động</option></select></td>
                                <td><select class="form-control" name="actualCondition_${rowKey}"><option value="">-</option><option value="GOOD" ${(empty item && asset.actualCondition == 'GOOD') || item.actualCondition == 'GOOD' ? 'selected' : ''}>Tốt</option><option value="FAIR" ${(empty item && asset.actualCondition == 'FAIR') || item.actualCondition == 'FAIR' ? 'selected' : ''}>Khá</option><option value="DAMAGED" ${(empty item && asset.actualCondition == 'DAMAGED') || item.actualCondition == 'DAMAGED' ? 'selected' : ''}>Hư hỏng</option><option value="BROKEN" ${(empty item && asset.actualCondition == 'BROKEN') || item.actualCondition == 'BROKEN' ? 'selected' : ''}>Không hoạt động</option></select></td>
                                <td><input class="form-control" name="discrepancyType_${rowKey}" value="<c:out value='${item.discrepancyType}'/>" placeholder="Thiếu, hư hỏng..."></td>
                                <td><input class="form-control" name="discrepancyNote_${rowKey}" value="<c:out value='${item.discrepancyNote}'/>" placeholder="Ghi chú tùy chọn"></td>
                            </tr>
                        </c:forEach>
                    </tbody></table></div>

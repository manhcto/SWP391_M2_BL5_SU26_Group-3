...
<div class="app-shell">
    ...
    <main class="main-content">
        <header class="topbar"><div class="heading-wrap"><button class="menu-button" id="menuButton" type="button" aria-label="Mở thanh điều hướng" aria-controls="sidebar" aria-expanded="false"><svg><use href="#i-menu"/></svg></button><div><h1>Thanh lý thiết bị</h1><p>Theo dõi các hồ sơ thanh lý trong phòng lab</p></div></div><div class="topbar-actions"><div class="top-profile"><div class="avatar">ME</div><span><c:out value="${currentUser.fullName}"/></span></div></div></header>
        <section class="content-area">
            <div class="content-heading"><div><p class="eyebrow">CỔNG NGƯỜI HƯỚNG DẪN</p><h2>Hồ sơ thanh lý</h2></div></div>
            ...
            <article class="panel"><c:choose><c:when test="${empty disposals}"><div class="empty-box"><div class="empty-box-icon"><svg><use href="#i-trash"/></svg></div><h3>Chưa có hồ sơ thanh lý</h3><p>Không có bản ghi phù hợp với bộ lọc.</p></div></c:when><c:otherwise><div class="table-scroll"><table><thead><tr><th>Mã thanh lý</th><th>Thiết bị</th><th>Lý do</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody><c:forEach items="${disposals}" var="d"><tr><td>#DSP-${d.disposalId}</td><td><strong><c:out value="${d.assetName}"/></strong><br><small><c:out value="${d.assetCode}"/><c:if test="${not empty d.assetItemTag}"> · <c:out value="${d.assetItemTag}"/></c:if></small></td><td><c:out value="${d.reason}"/></td><td><span class="status ${d.status == 'PENDING' ? 'review' : d.status == 'COMPLETED' ? 'returned' : 'open'}"><c:out value="${app:label(d.status)}"/></span></td><td><a class="btn-action btn-action-primary" href="${pageContext.request.contextPath}/mentor/disposals/${d.disposalId}">Xem</a></td></tr></c:forEach></tbody></table></div><div class="table-footer"><span>Hiển thị ${disposals.size()} hồ sơ thanh lý</span></div></c:otherwise></c:choose></article>
        </section>
    </main>
</div>
</body>

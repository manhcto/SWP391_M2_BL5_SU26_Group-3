    ...
    <main class="main-content">
        <header class="topbar">
            <div class="heading-wrap">
                ...
                <div>
                    <h1>Đánh giá trách nhiệm</h1>
                    <p>Ghi nhận trách nhiệm dựa trên kết luận kỹ thuật của Lab Manager.</p>
                </div>
            </div>
        ...
        <section class="content-area">
            <div class="content-heading">
                <div><p class="eyebrow">ĐÁNH GIÁ</p><h2>Hồ sơ trách nhiệm (${responsibilities.size()})</h2></div>
                <a class="primary-button" href="${pageContext.request.contextPath}/mentor/responsibilities/new">+ Thêm đánh giá</a>
            </div>

            <form class="filter-bar" method="get" action="${pageContext.request.contextPath}/mentor/responsibilities">
                <div class="filter-group">
                    <input class="form-control" type="search" name="keyword" value="<c:out value='${keyword}'/>" placeholder="Tìm theo mã, sự cố, Intern, thiết bị hoặc đánh giá" style="width:280px">
                ...
                <c:choose>
                    <c:when test="${empty responsibilities}">
                        <div class="empty-box">
                            ...
                            <p>Tạo đánh giá sau khi sự cố đã có kết luận kỹ thuật.</p>
                            <a class="primary-button" href="${pageContext.request.contextPath}/mentor/responsibilities/new">+ Thêm đánh giá</a>
                        </div>
                    </c:when>
                    <c:otherwise>
                            ...
                            <table>
                                ...
                                <c:forEach var="r" items="${responsibilities}">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${not empty r.responsibilityLevel}"><c:out value="${app:label(r.responsibilityLevel)}"/></c:when>
                                                <c:otherwise>Hồ sơ cũ</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${not empty r.internId}"><span class="student"><i>IN</i><c:out value="${r.internName}"/></span><br><small><c:out value="${r.internCode}"/></small></c:when>
                                                <c:otherwise>Không gán Intern</c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td class="wrap-cell">
                                            <c:choose><c:when test="${not empty r.evidenceSummary}"><c:out value="${r.evidenceSummary}"/></c:when><c:otherwise><c:out value="${r.conclusion}" default="—"/></c:otherwise></c:choose>
                                            <c:if test="${not empty r.responsibilityNote}"><br><small><c:out value="${r.responsibilityNote}"/></small></c:if>
                                        </td>
                                        ...
                                        <td class="actions-cell">
                                            <a class="btn-action" href="${pageContext.request.contextPath}/mentor/responsibilities/${r.responsibilityId}">Xem</a>
                                            <c:if test="${not empty r.technicalAssessedAt}"><a class="btn-action btn-action-primary" href="${pageContext.request.contextPath}/mentor/responsibilities/${r.responsibilityId}/edit">Sửa</a></c:if>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>

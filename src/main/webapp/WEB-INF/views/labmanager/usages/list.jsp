...
<div class="app-shell">
    ...
    <main class="main-content">
        ...
        <section class="content-area">
            ...
            <article class="panel"><c:choose><c:when test="${empty usages}"><div class="empty-box"><div class="empty-box-icon"><svg><use href="#i-calendar"/></svg></div><h3>Chưa có lịch sử sử dụng</h3><p>Không có bản ghi phù hợp với bộ lọc.</p></div></c:when><c:otherwise><div class="table-scroll"><table><thead><tr><th>Mã lượt sử dụng</th><th>Thực tập sinh</th><th>Thiết bị / sản phẩm</th><th>Intern báo cáo</th><th>Yêu cầu trả</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody><c:forEach items="${usages}" var="u"><tr><td>#AU-${u.assetUsageId}</td><td><c:out value="${u.internName}"/></td><td><strong><c:out value="${u.assetName}"/></strong><br><small><c:out value="${empty u.assetItemTag ? u.assetCode : u.assetItemTag}"/></small></td><td><c:out value="${empty u.reportedConditionAfter ? '-' : app:label(u.reportedConditionAfter)}"/></td><td><c:out value="${empty u.returnRequestedAt ? 'Chưa yêu cầu' : app:dateTime(u.returnRequestedAt)}"/></td><td><span class="status ${u.status == 'IN_USE' ? 'in-use' : (u.status == 'RETURN_PENDING' ? 'review' : 'returned')}"><c:out value="${app:label(u.status)}"/></span></td><td><a class="btn-action btn-action-primary" href="${pageContext.request.contextPath}/lab-manager/usages/${u.assetUsageId}">${u.status == 'RETURN_PENDING' ? 'Xác nhận' : 'Xem'}</a></td></tr></c:forEach></tbody></table></div><div class="table-footer"><span>Hiển thị ${usages.size()} lượt sử dụng</span></div></c:otherwise></c:choose></article>
        </section>
    </main>
</div>
</body>

        ...
        <header class="topbar">
            <div class="heading-wrap">
                ...
                <div>
                    ...
                    <p>Theo dõi tiến độ sửa chữa thiết bị hỏng và quản lý kế hoạch bảo dưỡng định kỳ phòng lab</p>
                </div>
            </div>
        ...
        <section class="content-area">
            ...
            <c:if test="${not empty dueSchedules}">
                <div class="alert-banner">
                    <div class="alert-banner-header">
                        <div style="font-size: 14.5px; font-weight: 700; color: #92400e; display: flex; align-items: center; gap: 8px;">
                            <span>NHẮC NHỞ LỊCH BẢO TRÌ ĐỊNH KỲ</span>
                            <span class="alert-badge">${dueSchedules.size()} lịch cần thực hiện</span>
                        </div>
                    ...
                    <div style="font-size: 13.5px; color: #78350f; line-height: 1.5;">
                        Phát hiện <b>${dueSchedules.size()}</b> thiết bị đến hạn bảo dưỡng trong tuần này hoặc đã quá hạn:
                        <ul style="margin: 6px 0 0 20px; padding: 0;">
                            <c:forEach var="ds" items="${dueSchedules}" varStatus="status">
                                <c:if test="${status.index < 3}">
                                    <li style="margin-bottom: 3px;">
                                        <b><c:out value="${ds.title}"/></b> — <c:out value="${ds.assetName}"/> (<c:out value="${not empty ds.itemCode ? ds.itemCode : ds.assetCode}"/>)
                                        <c:choose>
                                            <c:when test="${ds.overdue}">
                                                <span style="color: #dc2626; font-weight: 700;">(Đã quá hạn ${ds.daysDiff * -1} ngày)</span>
                                            </c:when>
                                            <c:when test="${ds.daysDiff == 0}">
                                                <span style="color: #ea580c; font-weight: 700;">(Đến hạn hôm nay!)</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span style="color: #b45309; font-weight: 600;">(Hạn: ${app:date(ds.scheduledDate)} - Còn ${ds.daysDiff} ngày)</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </li>
                                </c:if>
                            ...
                            <c:if test="${dueSchedules.size() > 3}">
                                <li><em>Và ${dueSchedules.size() - 3} thiết bị khác trong danh sách lịch...</em></li>
                            </c:if>
                        </ul>
                    </div>
                </div>
            ...
            <div class="content-heading">
                <div>
                    ...
                    <h2>
                        ${activeTab == 'schedules' ? 'Kế hoạch bảo trì định kỳ' : 'Danh sách phiếu bảo trì'}
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <c:choose>
                        <c:when test="${activeTab == 'schedules'}">
                            <a class="btn-secondary" style="height: 38px; padding: 0 14px; font-size: 13px; display: inline-flex; align-items: center; gap: 6px; text-decoration: none;"
                            ...
                            <a class="primary-button" href="${pageContext.request.contextPath}/lab-manager/maintenance/schedules/new">
                                <svg><use href="#i-plus"/></svg>+ Lên lịch bảo trì mới
                            </a>
                        </c:when>
                        <c:otherwise>
            ...
            <c:if test="${param.success == 'schedule_created'}">
                <div class="success-message">Đã tạo lịch bảo trì định kỳ mới thành công.</div>
            </c:if>
            <c:if test="${param.success == 'schedule_updated'}">
                <div class="success-message">Đã cập nhật thông tin lịch bảo trì thành công.</div>
            </c:if>
            <c:if test="${param.success == 'status_updated'}">
                <div class="success-message">Đã cập nhật trạng thái lịch bảo trì thành công.</div>
            </c:if>
            <c:if test="${param.success == 'schedule_deleted'}">
                <div class="success-message">Đã xóa lịch bảo trì định kỳ thành công.</div>
            </c:if>
            <c:if test="${param.success == 'batch_cancelled'}">
                <div class="success-message">Đã hủy các lịch bảo trì đã chọn thành công.</div>
            </c:if>
            <c:if test="${param.success == 'batch_deleted'}">
                <div class="success-message">Đã xóa các lịch bảo trì đã chọn khỏi hệ thống thành công.</div>
            </c:if>
            <c:if test="${param.success == 'imported'}">
                <div class="success-message">
                    Đã nhập thành công <b>${param.count}</b> lịch bảo trì định kỳ từ file Excel!
                    <c:if test="${not empty param.skipped and param.skipped > 0}">
                        <span style="color: #92400e; font-weight: 500; margin-left: 6px;">(Đã tự động bỏ qua <b>${param.skipped}</b> dòng bị trùng ngày với lịch đã có trên hệ thống).</span>
                    </c:if>
                </div>
            </c:if>
                ...
                <a class="nav-tab-item ${activeTab != 'schedules' ? 'active' : ''}"
                   href="${pageContext.request.contextPath}/lab-manager/maintenance?tab=tickets">
                    ...
                    <c:if test="${not empty records}">
                        <span style="background: #e2e8f0; color: #475569; font-size: 11.5px; padding: 2px 7px; border-radius: 10px;">${records.size()}</span>
                    </c:if>
                </a>
                <a class="nav-tab-item ${activeTab == 'schedules' ? 'active' : ''}"
            ...
            <c:choose>
                    ...
                    <form class="filter-bar" method="get" action="${pageContext.request.contextPath}/lab-manager/maintenance">
                        ...
                        <div class="filter-group">
                            <input class="form-control" type="search" name="keyword"
                                   ...
                                   placeholder="Tìm theo tiêu đề, tên thiết bị, kỹ thuật viên..." style="width:320px">
                            <select class="form-control" name="status">
                                <option value="">Tất cả trạng thái</option>
                                <option value="PENDING"   ${selectedStatus == 'PENDING'   ? 'selected' : ''}>Đang chờ thực hiện</option>
                                <option value="COMPLETED" ${selectedStatus == 'COMPLETED' ? 'selected' : ''}>Đã hoàn tất</option>
                            ...
                            <a class="btn-secondary" href="${pageContext.request.contextPath}/lab-manager/maintenance?tab=schedules">Đặt lại</a>
                        </div>
                    </form>
                    ...
                    <article class="panel">
                        <c:choose>
                            <c:when test="${empty schedules}">
                                <div class="empty-box">
                                    <div class="empty-box-icon"><svg><use href="#i-calendar"/></svg></div>
                                    <h3>Chưa có lịch bảo trì định kỳ nào</h3>
                                    <p>Hãy lên kế hoạch bảo dưỡng định kỳ trước cho các thiết bị phòng lab để luôn sẵn sàng hoạt động.</p>
                                    <a class="primary-button" href="${pageContext.request.contextPath}/lab-manager/maintenance/schedules/new">+ Lên lịch bảo trì mới</a>
                                </div>
                            </c:when>
                            <c:otherwise>
                                <form id="batchScheduleForm" method="post" action="${pageContext.request.contextPath}/lab-manager/maintenance" style="margin:0;">
                                    ...
                                    <div id="batchActionBar" style="display:none; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:10px 16px; margin-bottom:14px; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px;">
                                        <div style="font-size:13.5px; font-weight:600; color:#166534; display:flex; align-items:center; gap:8px;">
                                            <span>Đã chọn <b id="selectedCount" style="color:#15803d; font-size:15px;">0</b> lịch bảo trì</span>
                                        </div>
                                        <div style="display:flex; gap:8px; align-items:center;">
                                            <button type="submit" name="action" value="batchDeleteSchedules" class="btn-secondary"
                                            ...
                                            <button type="button" class="btn-secondary" style="height:30px; padding:0 10px; font-size:12px;" onclick="clearAllSelections()">
                                                Bỏ chọn
                                            </button>
                                        </div>
                                    </div>
                                        ...
                                        <table>
                                            ...
                                            <tr>
                                                <th style="width:40px; text-align:center;">
                                                    <input type="checkbox" id="selectAllSchedules" onclick="toggleSelectAll(this)" title="Chọn tất cả" style="cursor:pointer; width:15px; height:15px; vertical-align:middle;">
                                                </th>
                                                <th>Mã lịch</th>
                                                <th>Tiêu đề đợt bảo trì</th>
                                                <th>Thiết bị &amp; Vị trí</th>
                                                <th>Ngày dự kiến</th>
                                                <th>Đơn vị / Kỹ thuật viên</th>
                                                <th>Dự toán</th>
                                                <th>Trạng thái</th>
                                            ...
                                            <c:forEach var="s" items="${schedules}">
                                                <tr style="${s.overdue ? 'background:#fff5f5;' : (s.dueSoon ? 'background:#fffdf5;' : '')}">
                                                    <td style="text-align:center;">
                                                        <c:choose>
                                                            <c:when test="${s.status == 'COMPLETED'}">
                                                                <span style="color:#cbd5e1; font-size:15px; font-weight:bold;" title="Lịch đã hoàn tất (Không thể thao tác/xóa)">—</span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <input type="checkbox" name="selectedScheduleIds" value="${s.scheduleId}" class="schedule-checkbox" onchange="onCheckboxChange()" style="cursor:pointer; width:15px; height:15px; vertical-align:middle;">
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <td><strong>#SCH-<c:out value="${s.scheduleId}"/></strong></td>
                                                    <td>
                                                        <strong><c:out value="${s.title}"/></strong>
                                                        <c:if test="${not empty s.note}">
                                                            <small style="display:block;color:#64748b;margin-top:2px;"><c:out value="${s.note}"/></small>
                                                        </c:if>
                                                    </td>
                                                    <td>
                                                        <c:out value="${s.assetName}"/>
                                                        <small style="display:block;color:#5a6662">
                                                            Mã: <c:out value="${not empty s.itemCode ? s.itemCode : s.assetCode}"/>
                                                            <c:if test="${not empty s.storageLocation}"> • Vị trí: <c:out value="${s.storageLocation}"/></c:if>
                                                        </small>
                                                    </td>
                                                    <td>
                                                        ...
                                                        <c:if test="${s.status == 'PENDING'}">
                                                            <c:choose>
                                                                <c:when test="${s.overdue}">
                                                                    <small style="display:block;color:#dc2626;font-weight:700;">Quá hạn ${s.daysDiff * -1} ngày</small>
                                                                </c:when>
                                                                <c:when test="${s.daysDiff == 0}">
                                                                    <small style="display:block;color:#ea580c;font-weight:700;">Đến hạn hôm nay</small>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <small style="display:block;color:#64748b;">Còn ${s.daysDiff} ngày</small>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </c:if>
                                                    </td>
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${not empty s.providerName}">
                                                                <c:out value="${s.providerName}"/>
                                                                <c:if test="${not empty s.providerPhone}">
                                                                    <small style="display:block;color:#2563eb;"><c:out value="${s.providerPhone}"/></small>
                                                                </c:if>
                                                            </c:when>
                                                    ...
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${not empty s.estimatedCost}">
                                                                <strong style="color:#2563eb;"><fmt:formatNumber value="${s.estimatedCost}" type="number" groupingUsed="true"/> VNĐ</strong>
                                                            </c:when>
                                                            <c:otherwise><span style="color:#9ca3af;">—</span></c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <td>
                                                        <c:choose>
                                                            <c:when test="${s.status == 'COMPLETED'}">
                                                                <span class="schedule-badge-completed">Đã hoàn tất</span>
                                                            </c:when>
                                                            <c:when test="${s.status == 'CANCELLED'}">
                                                                <span class="schedule-badge-cancelled">Đã hủy</span>
                                                            </c:when>
                                                            <c:when test="${s.overdue}">
                                                                <span class="schedule-badge-overdue">Quá hạn</span>
                                                            </c:when>
                                                            <c:when test="${s.dueSoon}">
                                                                <span class="schedule-badge-duesoon">Sắp đến hạn</span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <span class="schedule-badge-pending">Đang chờ</span>
                                                            </c:otherwise>
                                                        </c:choose>
                                                    </td>
                                                    <td style="text-align:right;white-space:nowrap;">
                                                        <c:choose>
                                                            <c:when test="${s.status == 'PENDING'}">
                                                                <div class="single-row-actions" style="display:inline-flex; gap:6px;">
                                                                    <%-- Nút tạo phiếu bảo trì nhanh từ lịch --%>
                                                                    <a class="btn-secondary" style="height:24px;padding:0 8px;font-size:11px;background:#e0f2fe;color:#0284c7;border-color:#bae6fd;font-weight:600;"
                                                                       href="${pageContext.request.contextPath}/lab-manager/maintenance/new?assetId=${s.assetId}&itemCode=${s.itemCode}&scheduleId=${s.scheduleId}&note=${s.providerName}&providerPhone=${s.providerPhone}&estimatedCost=${s.estimatedCost}&description=${s.title}" title="Tạo phiếu bảo trì cho thiết bị này">
                                                                        + Tạo phiếu
                                                                    </a>
                                                                ...
                                                                <span class="batch-mode-dash" style="display:none; color:#cbd5e1; font-size:15px; font-weight:bold;">—</span>
                                                            </c:when>
                                                            <c:otherwise>
                                    ...
                                    <div class="table-footer" style="display:flex; align-items:center; justify-content:space-between; padding:12px 18px; border-top:1px solid #edf0ec; font-size:13px; color:#68736f; flex-wrap:wrap; gap:12px;">
                                        <div style="display:flex; align-items:center; gap:8px;">
                                            <span>Hiển thị</span>
                                            <select id="schedulePageSizeSelect" class="form-control" style="width:auto; height:32px; padding:4px 8px; font-size:13px;" onchange="changeSchedulePageSize(this.value)">
                                                <option value="5">5</option>
                                                <option value="10" selected>10</option>
                                                <option value="20">20</option>
                                                <option value="50">50</option>
                                            </select>
                                            <span>lịch bảo trì mỗi trang</span>
                                        </div>
                                        <span id="schedulePageInfoText">Hiển thị 1 đến ${schedules.size()} trong tổng số ${schedules.size()} lịch bảo trì</span>
                                        <div class="pagination-controls" id="schedulePaginationControls"></div>
                                    </div>
                                </form>
                            </c:otherwise>
                        </c:choose>
                ...
                <c:otherwise>
                    ...
                    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:20px;">
                        ...
                        <div style="background:#fff; border:1px solid #e5e7eb; border-radius:10px; padding:18px 20px;">
                            <div style="font-size:12px; color:#6b7280; font-weight:600; text-transform:uppercase; letter-spacing:.5px;">Tổng chi phí đã chi</div>
                    ...
                    <form class="filter-bar" method="get" action="${pageContext.request.contextPath}/lab-manager/maintenance">
                        ...
                        <div class="filter-group">
                            ...
                            <button class="primary-button" type="submit">Tìm kiếm</button>
                            <a class="btn-secondary" href="${pageContext.request.contextPath}/lab-manager/maintenance?tab=tickets">Đặt lại</a>
                        </div>
                    </form>
                                        ...
                                        <c:forEach var="r" items="${records}">
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <small style="display:block;color:#5a6662">
                                                        <c:out value="${not empty r.assetItemCode ? r.assetItemCode : r.assetCode}"/>
                                                    </small>
                                                </td>
                                                <td><c:out value="${r.quantity}"/></td>
                                                <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><c:out value="${r.description}"/></td>
                                                <td>
                                                    <c:choose>
                                                ...
                                                <td style="white-space:nowrap;">
                                                    <a class="btn-secondary" style="height:24px;padding:0 8px;font-size:11px" href="${pageContext.request.contextPath}/lab-manager/maintenance/${r.maintenanceId}">Xem</a>
...
<%-- MODAL IMPORT EXCEL --%>
<div id="importScheduleModal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.6); backdrop-filter:blur(2px); z-index:9999; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:12px; width:100%; max-width:520px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.2); overflow:hidden; animation: fadeIn .15s ease;">
        ...
        <form method="post" action="${pageContext.request.contextPath}/lab-manager/maintenance" enctype="multipart/form-data" style="margin:0;">
            ...
            <div style="padding:24px;">
                <p style="margin:0 0 16px 0; font-size:13.5px; color:#64748b; line-height:1.5;">
                    Chọn tệp tin định dạng <code>.xlsx</code> chứa danh sách kế hoạch bảo trì. Nếu bạn chưa có file mẫu chuẩn, vui lòng tải file mẫu bên dưới.
                </p>
                <div style="margin-bottom:18px; padding:12px 16px; background:#eff6ff; border:1px solid #bfdbfe; border-radius:8px; display:flex; justify-content:space-between; align-items:center;">
                    <span style="font-size:13px; color:#1e40af; font-weight:600;">File mẫu có sẵn cột &amp; mã máy:</span>
                    <a href="${pageContext.request.contextPath}/lab-manager/maintenance/schedules/template" style="font-size:12.5px; color:#2563eb; font-weight:700; text-decoration:none;">
                        Tải file mẫu .xlsx
                    </a>
                </div>
                <div style="border:2px dashed #cbd5e1; border-radius:8px; padding:24px 16px; text-align:center; background:#f8fafc;">
                    <input type="file" name="excelFile" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required style="font-size:13.5px; color:#334155;">
                    <small style="display:block; margin-top:8px; font-size:12px; color:#94a3b8;">Hỗ trợ Microsoft Excel (.xlsx)</small>
                </div>
            </div>
            <div style="padding:14px 24px; background:#f8fafc; border-top:1px solid #f1f5f9; display:flex; justify-content:flex-end; gap:10px;">
                <button type="button" class="btn-secondary" onclick="document.getElementById('importScheduleModal').style.display='none'" style="height:36px; padding:0 16px;">Hủy</button>
                <button type="submit" class="primary-button" style="height:36px; padding:0 18px;">Bắt đầu nạp dữ liệu</button>
            </div>
        </form>
    </div>
</div>

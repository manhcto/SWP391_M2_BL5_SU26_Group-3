                    ...
                    <div class="asset-info">
                        ...
                        <c:choose><c:when test="${editMode}"><form method="post" action="${assetBasePath}/${item.assetItemId}" style="margin-top:16px">
                            <div class="form-grid">
                                ...
                                <div class="form-group"><label for="status">Trạng thái</label><select class="form-control" id="status" name="status"><option value="AVAILABLE" ${item.status == 'AVAILABLE' ? 'selected' : ''}>Sẵn sàng</option><option value="MAINTENANCE" ${item.status == 'MAINTENANCE' ? 'selected' : ''}>Đang bảo trì</option><option value="UNAVAILABLE" ${item.status == 'UNAVAILABLE' ? 'selected' : ''}>Không khả dụng</option><option value="DISPOSED" ${item.status == 'DISPOSED' ? 'selected' : ''}>Đã thanh lý</option></select></div>
                                <div class="form-group"><label for="purchaseDate">Ngày mua</label><input class="form-control" id="purchaseDate" name="purchaseDate" type="date" value="${item.purchaseDate}"></div>
                                <div class="form-group"><label for="warrantyUntil">Bảo hành đến</label><input class="form-control" id="warrantyUntil" name="warrantyUntil" type="date" value="${item.warrantyUntil}"></div>
                        ...
                        </form></c:when><c:otherwise><dl class="asset-readonly"><dt>Serial</dt><dd><c:out value="${empty item.serialNumber ? 'Chưa nhập' : item.serialNumber}"/></dd><dt>Tình trạng</dt><dd><c:out value="${app:label(item.condition)}"/></dd><dt>Trạng thái</dt><dd><c:out value="${app:label(item.status)}"/></dd><dt>Ngày mua</dt><dd><c:out value="${empty item.purchaseDate ? 'Chưa nhập' : item.purchaseDate}"/></dd><dt>Bảo hành đến</dt><dd><c:out value="${empty item.warrantyUntil ? 'Chưa nhập' : item.warrantyUntil}"/></dd><dt>Ghi chú</dt><dd><c:out value="${empty item.note ? 'Không có' : item.note}"/></dd><div class="form-actions"><a class="primary-button" href="${assetBasePath}/${item.assetItemId}/edit">Sửa</a><form method="post" action="${assetBasePath}/${item.assetItemId}/delete" onsubmit="return confirm('Xóa sản phẩm này? Nếu sản phẩm đã có lịch sử sử dụng, hệ thống sẽ từ chối.');"><input type="hidden" name="csrfToken" value="${csrfToken}"><button class="btn-action btn-action-danger" type="submit">Xóa</button></form></div></dl></c:otherwise></c:choose>
                    </div>
                </div>
            </article>
        </section>

    ...
    <main class="main-content">
        ...
        <section class="content-area">
            ...
            <form method="post" enctype="multipart/form-data" action="${assetBasePath}/new">
                ...
                <article class="panel form-section" style="margin-top:12px"><header class="panel-header"><div class="panel-title"><span class="title-icon"><svg><use href="#i-list"/></svg></span><h3>Thông tin từng sản phẩm</h3></div></header><p class="form-note">Chọn ảnh từ máy tính cho từng sản phẩm. Hệ thống lưu ảnh riêng và xem trước ngay tại đây; chấp nhận JPG, PNG hoặc WebP, tối đa 5 MB.</p><div class="table-scroll"><table class="item-table"><thead><tr><th>#</th><th>Serial</th><th>Ảnh thiết bị</th><th>Tình trạng</th><th>Trạng thái</th><th>Ngày mua</th><th>Bảo hành đến</th><th>Ghi chú</th></tr></thead><tbody id="itemRows"></tbody></table></div></article>
...
<script>
const rowHost=document.getElementById('itemRows'), quantityInput=document.getElementById('quantity'), imageIcon='<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="2"></rect><circle cx="8.5" cy="9" r="1.5"></circle><path d="m4 18 5-5 3.5 3.5 2.5-2.5 5 4"></path></svg>';
function row(index){const imageId='itemImageFile'+index;return '<tr><td>'+(index+1)+'</td><td><input name="itemSerialNumber" maxlength="100" placeholder="Serial"></td><td class="item-image-cell"><input class="item-image-file" id="'+imageId+'" name="'+imageId+'" type="file" accept="image/jpeg,image/png,image/webp"><label class="image-picker" for="'+imageId+'"><span class="image-picker-preview">'+imageIcon+'</span><span class="image-picker-copy">Chọn ảnh<small>Hiển thị xem trước</small></span></label><p class="image-file-note">JPG, PNG, WebP · tối đa 5 MB</p></td><td><select name="itemCondition"><option value="GOOD">Tốt</option><option value="FAIR">Khá</option><option value="DAMAGED">Hư hỏng</option><option value="BROKEN">Hỏng</option></select></td><td><select name="itemStatus"><option value="AVAILABLE">Sẵn sàng</option><option value="MAINTENANCE">Đang bảo trì</option><option value="UNAVAILABLE">Không khả dụng</option><option value="DISPOSED">Đã thanh lý</option></select></td><td><input name="itemPurchaseDate" type="date"></td><td><input name="itemWarrantyUntil" type="date"></td><td><input name="itemNote" maxlength="500" placeholder="Ghi chú"></td></tr>';}
...
rowHost.addEventListener('change',event=>{const input=event.target;if(!input.matches('.item-image-file'))return;const preview=input.parentElement.querySelector('.image-picker-preview'),file=input.files[0];if(!file)return;if(!['image/jpeg','image/png','image/webp'].includes(file.type)||file.size>5*1024*1024){input.value='';preview.innerHTML=imageIcon;alert('Ảnh phải là JPG, PNG hoặc WebP và không vượt quá 5 MB.');return;}const image=document.createElement('img'),url=URL.createObjectURL(file);image.onload=()=>URL.revokeObjectURL(url);image.src=url;image.alt='Xem trước ảnh thiết bị';preview.replaceChildren(image);});
renderRows();
</script>
</body>
</html>

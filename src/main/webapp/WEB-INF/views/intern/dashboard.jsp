...
<head>
    ...
    <title>Bảng điều khiển INTERN | LAB Asset</title>
    ...
    <main class="main-content">
        <header class="topbar">
            <div class="heading-wrap">
                ...
                <div><h1>Bảng điều khiển INTERN</h1>
                    <p>Theo dõi thiết bị, việc trả thiết bị và hồ sơ trách nhiệm</p></div>
            </div>
        ...
        <section class="content-area">
            <div class="content-heading">
                <div><p class="eyebrow">CỔNG INTERN</p>
                    <h2>Xin chào, <c:out value="${currentUser.fullName}"/></h2></div>
                <c:if test="${permissions.allows('ASSET_USAGE_BORROW')}"><a class="primary-button"
                    ...
                    Mượn thiết bị</a></c:if>
            </div>
            <section class="stats-grid" aria-label="Tổng quan hoạt động tại phòng LAB">
                <a class="stat-card stat-blue" href="${pageContext.request.contextPath}/intern/usages">
                    ...
                    <div><strong><c:out value="${activeUsageCount}"/></strong><span>Thiết bị đang mượn</span><small>Thiết
                        bị đang được sử dụng</small></div>
                </a>
                <a class="stat-card stat-green" href="${pageContext.request.contextPath}/intern/usages">
                    ...
                    <div><strong><c:out value="${returnedUsageCount}"/></strong><span>Thiết bị đã trả</span><small>Các
                        lượt trả đã hoàn tất</small></div>
                </a>
                ...
                <a class="stat-card stat-purple" href="${pageContext.request.contextPath}/intern/usages">
                    ...
                    <div><strong><c:out value="${totalUsageCount}"/></strong><span>Tổng lượt sử dụng</span><small>Tất cả
                        lịch sử mượn</small></div>
                </a>
            </section>
            <section class="dashboard-grid intern-dashboard-grid">
                <article class="panel">
                    <header class="panel-header">
                        <div class="panel-title"><span class="title-icon"><svg><use href="#i-calendar"/></svg></span>
                            <h3>Sử dụng thiết bị gần đây</h3></div>
                        <a href="${pageContext.request.contextPath}/intern/usages">Xem tất cả</a></header>
                        ...
                        <table>
                            ...
                            <c:choose>
                                <c:when test="${empty usages}">
                                    <tr>
                                        <td colspan="6">Chưa có lịch sử sử dụng thiết bị.</td>
                                    </tr>
                                </c:when>
                                <c:otherwise><c:forEach var="usage" items="${usages}" end="4">
                                    <tr>
                                        <td><strong><c:out value="${usage.assetName}"/></strong><br><small><c:out
                                                value="${usage.assetCode}"/></small></td>
                ...
                <article class="panel">
                    <header class="panel-header">
                        <div class="panel-title"><span class="title-icon"><svg><use href="#i-grid"/></svg></span>
                            <h3>Thao tác nhanh</h3></div>
                    </header>
                    <div class="intern-actions">
                        <c:if test="${permissions.allows('ASSET_USAGE_BORROW')}"><a class="intern-action"
                                                                                    href="${pageContext.request.contextPath}/intern/usages/borrow"><span
                                class="intern-action-icon"><svg><use href="#i-box"/></svg></span><span><strong>Mượn thiết bị</strong><small>Chọn thiết bị sẵn sàng trong phòng LAB</small></span></a></c:if>

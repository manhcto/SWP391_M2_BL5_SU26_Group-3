...
<div class="app-shell">
    ...
    <main class="main-content">
        ...
        <section class="content-area">
            ...
            <c:if test="${not empty assets}">
                <article class="panel">
                    ...
                    <form method="post" action="${pageContext.request.contextPath}/intern/usages/borrow">
                        ...
                        <div class="form-grid">
                            <div class="form-group full-width"><label for="assetId">Thiết bị</label><select
                                    class="form-control" id="assetId" name="assetId" required>
                                <option value="">Chọn thiết bị</option>
                                <c:forEach items="${assets}" var="asset">
                                    <option value="${asset.assetId}" ${param.assetId == asset.assetId ? 'selected' : ''}><c:out
                                            value="${asset.assetCode}"/> · <c:out value="${asset.assetName}"/> · <c:out
                                            value="${asset.totalQuantity}"/> tổng cộng</option>
                                </c:forEach>
                            </select></div>
                            <div class="form-group"><label for="quantity">Số lượng</label><input class="form-control"
                                                                                                      id="quantity" type="number"
                                                                                                      name="quantity" min="1"
                                                                                                      value="${empty param.quantity ? 1 : param.quantity}" required></div>
            ...
            <c:if test="${not empty assetItems}">
                <article class="panel">
                    ...
                    <form method="post" action="${pageContext.request.contextPath}/intern/usages/borrow">
                        ...
                        <div class="form-grid">
                            <div class="form-group full-width"><label for="assetItemId">Thiết bị</label><select
                                ...
                                <c:forEach items="${assetItems}" var="item">
                                    <option value="${item.assetItemId}" ${param.assetItemId == item.assetItemId ? 'selected' : ''}><c:out
                                            value="${item.assetCode}"/> · <c:out value="${item.assetName}"/> · <c:out
                                            value="${item.itemTag}"/></option>
                                </c:forEach>
                            </select></div>
                            <div class="form-group"><label>Số lượng</label><input class="form-control readonly-field"
            ...
            <c:if test="${empty assets && empty assetItems}">
